...
- **Request 1 (left):** the Prompt above, verbatim, then: "**Output 21:9 landscape, opaque, no text, no people; the bottom third is an empty flat ground plane.**" Save as `docs/refs/locations/07-basketball-gym/gen-left.png`.
- **Request 2 (right), same session, request-1 image attached:** "**Continue this exact scene to the right as a 21:9 image: the left third of the new image repeats the right third of the attached image exactly, then the same place goes on to the right with new details in the same style, light and horizon. No text, no people, the bottom third stays an empty flat ground plane.**" Save as `docs/refs/locations/07-basketball-gym/gen-right.png`.
- **Then:**
  ```bash
  node tools/stitch-backdrop.mjs docs/refs/locations/07-basketball-gym/gen-left.png docs/refs/locations/07-basketball-gym/gen-right.png docs/refs/locations/07-basketball-gym/gen-plate.png
  node tools/place-backdrop.mjs 7 docs/refs/locations/07-basketball-gym/gen-plate.png --floor=<measured %, e.g. 66%>
  npm run build:assets && npm run test:assets
  ```
- **Accept when:** the built `public/game/levels/bg-07.webp` shows the place described above across its whole width with no
...
- 2026-09-14 02:35 — **Second Midjourney plate** `docs/backdrops_midjourney/basketbass-gym.png` (≈2:1, bigger ground area) accepted: `place-backdrop 7 … --floor=46%` cuts an exact 4:1 window at full width with the ground line on the lane — no tiling, full five-wave coverage. Supersedes the first plate.
