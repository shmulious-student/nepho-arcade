...
export class Backdrop {
  ...
  constructor(scene: Phaser.Scene, level: LevelEntry, _worldWidth: number, container: Phaser.GameObjects.Container, campaignIndex = level.index, difficultyLabel = '') {
    this.scene = scene; this.level = level; this.campaignIndex = campaignIndex; this.difficultyLabel = difficultyLabel;
  ...
  showEntry(): void {
    ...
    const kicker = this.scene.add.text(cx, cy - 50, `LEVEL ${this.campaignIndex}${this.difficultyLabel ? ` · ${this.difficultyLabel}` : ''}`, { fontFamily: 'monospace', fontSize: '14px', color: Phaser.Display.Color.IntegerToColor(accent).rgba, letterSpacing: 6 } as Phaser.Types.GameObjects.Text.TextStyle).setOrigin(0.5);
