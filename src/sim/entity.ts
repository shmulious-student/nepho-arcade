...
export function makeEntity(id: number, kind: Kind, arch: string, x: number, y: number, hp: number): Entity {
  return {
    ...
    combo: 0, comboTimer: 0, comboStep: 0, cooldown: 0, ai: 0, aiT: 0, target: -1, dead: false, removeAt: 0,
...
export const isHurt = (e: Entity) => e.state === 'hurt' || e.state === 'hurtHeavy' || isDown(e);
